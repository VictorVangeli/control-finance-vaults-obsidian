...
>[!TIP] Это можно было бы упростить до 3-х шагов, добившись автоматической инициаилизации библиотеки, но это бы немного усложнило на мой взгляд процесс взаимодействия пользователя с добавлением полей внутри модальной формы. Но в случае если вам интересно - я могу выпустить альтернативный релиз
...
1. При открытии хранилища по умолчанию открывается файл [[Учет финансов]], в котором отражается текущая информация по доходам/расходам
	![[Pasted image 20241108150810.png|700]]
1. Логически структура файла разделена на 2 блока:
	- Фильтрации по критериям;
	- Таблицы, в которых отображаются все операции;
2. Структура дирректорий (дерево) устроена таким образом:
```
...
Выбор такой структуры обусловлен тем, чтобы минимизировать взаимодействие пользователя с формами `Modal Forms` и иметь возможность добавлять нужные критерии в разделы `Категории трат` и `Счет` созданием пустых заметок в соответствующих директориях с необходимым названием. 
	![[Pasted image 20241108153125.png|400]]
	...
	![[Pasted image 20241108153150.png|400]]
...
1. Вызвать модальное окно, находящееся в правом верхнем углу (иконка `Accesability`)![[Pasted image 20241108155317.png|300]]
2. В появившемся окне доступны следующие поля: 
>[!info]+ Примечание о полях
>Чтобы добавить свое поле не обращаясь к управлению формами в Modal Forms вам необходимо перейти в дирректорию: `Учет финансов/Utils/Словарь ключей` и отредактировать содержимое этих папок, добавив или удалив необходимые значения 
- Тип: Доход, Расход
...
![[Pasted image 20241108205635.png|700]]
1. После того как вы заполнили шаблон необходимой информацией будет создан соответствующий файл (заметка) в дирректории `Учет финансов/Доходы/Расходы(в зависимости от того, что выбрали)/название категории`, о чем система отправит соответствующее уведомление
![[Pasted image 20241108160204.png|400]]
![[Pasted image 20241108163448.png|400]]
...
>[!info]- Пример содержимого файла с операцией
>![[Pasted image 20241108163557.png]]

Также вы можете увидеть изменения в файле `Учет финансов`
![[Pasted image 20241108163811.png|500]]
...
![[Pasted image 20241108164344.png|500]]
...
![[Pasted image 20241108181520.png|500]]
...
![[Pasted image 20241108195521.png]]
...
![[Pasted image 20241108200957.png|700]]
...
![[Pasted image 20241108202308.png]]
...
![[Pasted image 20241108202526.png|700]]
![[Pasted image 20241108202733.png|700]]
...
![[Pasted image 20241108203528.png|700]]
![[Pasted image 20241108203720.png|700]]
...
![[Pasted image 20241108204330.png|700]]
![[Pasted image 20241108204351.png|700]]
...
![[Pasted image 20241108205002.png|700]]
- или нажать на одну из двух кнопок на боковых панелях

  ![[Pasted image 20241108204933.png|300]]
...
![[Pasted image 20241108205401.png|700]]